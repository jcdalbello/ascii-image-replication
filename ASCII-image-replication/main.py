# 
# Load an image and reproduce it on the terminal with ASCII symbols.
# On resolutions with more than 800 pixels by side start to look croped on the console.
...
def get_brightness_values(image_data):
    """
    Takes a list of tuples, each tuple containing RGB values, and returns a list with the brightness value of every tuplet.  
    """
...
def print_ascii_image(
        ...
        brightness_scale):
    """
    Takes the list created with get_brightness_values(), the width of the image (int) and the brightness scale (string) selected by the user to print the ASCII reproduction of the original image on console.
    """
